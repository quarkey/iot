...
func (s *Server) InsertTestdata() error {
	...
	testdata := []query{
		...
		{"adding dataset 1", `insert into datasets(sensor_id, title, description, reference, intervalsec, fields, types) 
		values(1,'temp&hydro','Temperatur/hydro measurement, growhouse 1','8a1bbddba98a8d8512787d311352d951',2,'["temp", "hydro"]' ,'["float", "float"]');`},
		...
		{"adding dataset 2", `insert into datasets(sensor_id, title, description, reference, intervalsec, fields, types) 
		values(2,'Bicycle to work','Battery-driven lat/long tracker','4987fb174ae91dc702394024378fc1cd',1,'["lat (n)", "long (e)", "direction"]', '["float", "float", "string"]');`},
		...
		{"adding dataset 2", `insert into datasets(sensor_id, title, description, reference, intervalsec, fields, types) 
		values(2,'Bicycle to work','Battery-driven lat/long tracker','dummykey',1,'["lat (n)", "long (e)", "direction"]','["float","float","string"]');`},
		{"data point 1", `insert into sensordata(sensor_id, dataset_id, data) values(1,currval(pg_get_serial_sequence('datasets', 'id')),'["58.8533","5.7329","e"]');`},
	}
